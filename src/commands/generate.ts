...
import { resolveFileOrUrl, resolveFilesOrUrls } from '../utils/upload.js'
...
export function createGenerateCommand(): Command {
  ...
  generate
    ...
    .option('--image-urls <urls>', 'Comma-separated reference image URLs for image-to-image')
    ...
    .action(async (options) => {
      ...
      const imageUrls = await resolveFilesOrUrls(client, parseList(options.imageUrls))
  ...
  generate
    ...
    .action(async (options) => {
      ...
      let firstImageUrl = options.firstFrameUrl || options.firstImageUrl || undefined
      let lastImageUrl = options.lastFrameUrl || options.lastImageUrl || undefined
      const imageUrls = parseList(options.imageUrls)
      const videoUrls = parseList(options.videoUrls)
      if (options.referenceImageUrl) imageUrls.push(options.referenceImageUrl)
      if (options.referenceVideoUrl) videoUrls.push(options.referenceVideoUrl)
      ...
      if (firstImageUrl) {
        firstImageUrl = await resolveFileOrUrl(client, firstImageUrl)
      }
      if (lastImageUrl) {
        lastImageUrl = await resolveFileOrUrl(client, lastImageUrl)
      }
      const resolvedImageUrls = await resolveFilesOrUrls(client, imageUrls)
      const resolvedVideoUrls = await resolveFilesOrUrls(client, videoUrls)
      const resolvedAudioUrls = await resolveFilesOrUrls(client, parseList(options.audioUrls))
      let audioUrl = options.audioUrl
      if (audioUrl) {
        audioUrl = await resolveFileOrUrl(client, audioUrl)
      }

      let mode = options.mode as string | undefined
      if (!mode) {
        if (lastImageUrl) mode = 'first-last'
        else if (firstImageUrl) mode = 'first'
        else if (resolvedImageUrls.length || resolvedVideoUrls.length) mode = 'reference'
        else mode = 'text'
      }
      if (!VIDEO_MODES.has(mode)) {
        throw new CliError(
          `Unsupported video mode "${mode}". Use one of: ${[...VIDEO_MODES].join(', ')}.`
        )
      }
      ...
      if (mode === 'first-last' && (!firstImageUrl || !lastImageUrl)) {
        throw new CliError(
          'Missing --first-frame-url and --last-frame-url for first-last video mode.'
        )
      }
      if (mode === 'reference' && !resolvedImageUrls.length && !resolvedVideoUrls.length) {
        throw new CliError(
          'Missing reference media. Pass --reference-url, --image-urls, or --video-urls.'
        )
      }
      ...
      if (!providerId || !modelId || !channel || !ratio || !duration || !resolution) {
        ...
        const selected =
          ...
          ) || catalog.models.find((model) => model.mode === mode)
