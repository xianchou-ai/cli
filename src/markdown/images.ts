...
import { downloadFile, XianchouClient } from '../api/client.js'
import type { MarkdownImagePlanItem } from '../api/types.js'
...
export async function runMarkdownImages(
  ...
  const insertions: ImageInsertion[] = []
  for (const [index, item] of plan.items.entries()) {
    ...
    if (!url) throw new CliError(`No image URL returned for ${item.title}`)
...
async function generateAndPoll(
  ...
  while (true) {
    ...
    await new Promise((resolve) =>
      setTimeout(resolve, task.poll_interval ?? 3000)
    )
  }
}
...
function buildFileName(
  slug: string,
  item: MarkdownImagePlanItem,
  index: number
): string {
  if (item.kind === 'cover') return `${slug}-cover.webp`
